...
const canvas = document.getElementById("text") as HTMLCanvasElement
...
function extractTime(time: string) {
	...
	if (sixteenth > 0) sixteenth -= 1
	tick = tick || 0
	return tickFromBST(bar, sixteenth, tick)
}

function floorAndPad(num: number) {
	...
	if (currentEntry) {
		...
		currentLyricChars =
			currentEntry.chars !== undefined
				? currentEntry.chars
				: currentLyricChars
		if (currentLyricChars === -1) currentLyricChars = currentLyric.length
